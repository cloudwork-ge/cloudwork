...
export class Authuser {
  ...
  public static unreadChats: number[] = [];
  static checkUser() {
  ...
  static async getUserData(
    ...
    await http
      .post(commonService.baseUrl + "Users/GetUserData", null, headers)
      .toPromise()
      .then(
        (data) => {
          ...
          if (onSuccess != null)
            onSuccess(data["rootElement"]["DATA"].userData);
        },
        (err) => {
  ...
  static getUnreadChats() {
    var http = AppInjector.get(HttpClient);
    var commonService = AppInjector.get(CommonService);
    var options = commonService.getHttpOptions();
    return http
      .post(commonService.baseUrl + "Users/GetUnreadChats", {}, options)
      .pipe(
        tap(
          (data) => {},
          (error) => {
